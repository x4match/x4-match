    ...
    >
      <svg width="67" height="575" viewBox="0 0 67 575" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          ...
          stroke="url(#paint0_linear_register)"
          strokeMiterlimit="10"
        />
        <path
          ...
          stroke="url(#paint1_linear_register)"
          strokeMiterlimit="10"
        />
        <path
          ...
          stroke="url(#paint2_linear_register)"
          strokeWidth="0.75"
          strokeMiterlimit="10"
        />
        <path
        ...
        <path
          ...
          stroke="url(#paint5_linear_register)"
          strokeWidth="0.75"
          strokeMiterlimit="10"
        />
        <path
          ...
          stroke="url(#paint6_linear_register)"
          strokeWidth="0.75"
          strokeMiterlimit="10"
        />
        <defs>
...
const inputClass =
  'h-12 w-full rounded-2xl border border-white/15 bg-transparent pl-11 pr-4 text-sm text-white outline-none transition-colors placeholder:text-white/40 focus:border-(--login-accent)';
...
function RegisterForm() {
  ...
  const socialSignup = googleSignup || appleSignup;
  const busy = loading || googleLoading || appleLoading;
  const nextPath = params.get('next') || '/perfil';

  useEffect(() => {
    const pendingApple = getPendingAppleSignup();
    if (pendingApple) {
      setAppleSignup(true);
      if (pendingApple.email) setEmail(pendingApple.email);
      if (pendingApple.fullName) setName(pendingApple.fullName);
      return;
    }
    const pendingGoogle = getPendingGoogleSignup();
    if (!pendingGoogle) return;
    setGoogleSignup(true);
    if (pendingGoogle.email) setEmail(pendingGoogle.email);
    if (pendingGoogle.fullName) setName(pendingGoogle.fullName);
  }, []);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const pendingApple = appleSignup ? getPendingAppleSignup() : null;
    const pendingGoogle = googleSignup ? getPendingGoogleSignup() : null;
    if (appleSignup && !pendingApple?.identityToken) {
      setError('Se perdió la sesión de Apple. Volvé a iniciar con Apple.');
      return;
    }
    if (googleSignup && !pendingGoogle?.idToken) {
      setError('Se perdió la sesión de Google. Volvé a iniciar con Google.');
      return;
    }
    ...
    try {
      const response = await api.post('/auth/register', {
        ...
        ...(pendingApple?.identityToken
          ? { identityToken: pendingApple.identityToken }
          : pendingGoogle?.idToken
            ? { idToken: pendingGoogle.idToken, photo: pendingGoogle.photo }
            : { password }),
  ...
  async function onGoogleLogin() {
    ...
    try {
      ...
      if (response.data?.needsRegistration) {
        ...
        if (response.data.fullName) setName(response.data.fullName);
        return;
      }
  ...
  async function onAppleLogin() {
    ...
    try {
      ...
      if (response.data?.needsRegistration) {
        ...
        if (response.data.email || appleEmail) setEmail(response.data.email || appleEmail || '');
        if (response.data.fullName || fullName) setName(response.data.fullName || fullName || '');
        return;
      }
      finishClubLogin(response.data.access_token as string, response.data.user as AuthUser);
    } catch (err: unknown) {
      ...
      <section className="relative flex w-full flex-col justify-center overflow-hidden px-6 py-12 sm:px-10 lg:w-1/2 lg:px-14 xl:px-20">
        ...
        <div className="relative z-10 mx-auto w-full max-w-md sm:pl-12">
          ...
          <p className="mt-3 max-w-sm text-sm leading-relaxed text-white/70 sm:text-[15px]">
            {socialSignup
              ? `Completá tus datos para terminar el registro con ${appleSignup ? 'Apple' : 'Google'}.`
              : 'Creá tu cuenta de club para gestionar turnos, facturación y más'}
          </p>

          <form onSubmit={onSubmit} className="mt-9 space-y-3.5" style={{ ['--login-accent' as string]: ACCENT }}>
            <label className="relative block">
              <span className="sr-only">Nombre del responsable</span>
              <User className="pointer-events-none absolute left-4 top-1/2 size-4.5 -translate-y-1/2 text-white/40" />
              <input
                type="text"
                autoComplete="name"
                required
